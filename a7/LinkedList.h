...
typedef struct ll_iter {
    ...
    LinkedListNodePtr curNode;  // the node we are at, or NULL if broken
} LLIterSt, *LLIter;
...
// See:
// http://www.cs.yale.edu/homes/aspnes/classes/223/notes.html#functionPointers
...
int DestroyLinkedList(LinkedList list,
                      LLPayloadFreeFnPtr payloadFreeFunc);
...
//
// INPUT: A pointer to the LinkedList.
// INPUT: A pointer to a pointer that will be updated with the payload at the
//        tail of the list.
//
// Returns 0 if the slice was successful; non-zero for failure.
int SliceLinkedList(LinkedList list, void** dataPtr);
...
void SortLinkedList(LinkedList list, unsigned int ascending,
                    LLPayloadComparatorFnPtr comparator);

// ======================================================
// LLIter: A Linked List Iterator
// ======================================================

// Creates a iterator for the given linkedList. It is the customer's
// responsibility to ensure that the iterator is destroyed before modifying the
// LinkedList. The customer also needs to call DestroyLLIter to free it when the
// customer is done.
//
...
// Steps the iterator to the previous element in the LinkedList if there is one.
//
// INPUT: A pointer to an existing iterators.
//
// Returns 0 if successful; 1 otherwise.
...
//
// Returns 0 if the iterator was successfully destroyed and freed.
...
// Delete the node the iterator is pointing to.  After deletion, the iterator is
// either:
//
// - invalid and cannot be used (but must be freed), if there was only one
// element in the list
// - pointing to the successor of the deleted node, if there is one.
// - pointing to the predecessor of the deleted node, if the iterator was
// pointing at the tail before LLIterDelete was called.
//
...
// to. (If you want to insert at the end of a list, use AppendLinkedList.)
//
...
// 0 on success; the iterator still points to the same node, not to the inserted
// node.
